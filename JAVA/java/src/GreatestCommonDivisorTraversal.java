...
class GreatestCommonDivisorTraversal {
    ...
    public GreatestCommonDivisorTraversal(int n) {
        id = new int[n];
        sz = new int[n];
        for (int i = 0; i < n; ++i)
            id[i] = i;
        ...
        for (int i = 0; i < n; ++i)
            for (final int primeFactor : getPrimeFactors(nums[i], minPrimeFactors))
                if (primeToFirstIndex.containsKey(primeFactor))
                    unionBySize(primeToFirstIndex.get(primeFactor), i);
                else
    ...
    public static void main(String[] args) {
        GreatestCommonDivisorTraversal ob = new GreatestCommonDivisorTraversal(3);
        System.out.println(ob.canTraverseAllPairs(new int[]{3, 9, 5}));
    }
}
